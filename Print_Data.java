...
class Print_Data implements ActionListener{
    ...
    Print_Data(String e_id){
        try{
            Conn con = new Conn();
            String str = "select * from emp.employee";
            ...
            while(rs.next()){
                ...
                name = rs.getString("a");
                father = rs.getString("bb");
                age = rs.getString("c");
                dob = rs.getString("d");
                address = rs.getString("e");
                phone = rs.getString("ff");
                email = rs.getString("g");
                education = rs.getString("h");
                post = rs.getString("i");
                nid = rs.getString("j");
                emp_id = rs.getString("k");
            }
        }catch(Exception e){
            e.printStackTrace();
        }
        ...
        id9.setLayout(null);
        //ImageIcon img=new ImageIcon(getClass().getResource("log.jpg"));
        //id9.setIcon(img);
